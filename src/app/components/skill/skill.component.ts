...
export class SkillComponent {
  ...
  openDeleteSkillSwall(index: number) {
    Swal.fire({
      title: 'Eliminar',
      text:
        '¿Desea eliminar el skill ' +
        this.portfolioData.skills[index].lenguaje +
        '?',
    }).then((result) => {
      if (result.isConfirmed) {
        const userId = this.loginService.getUserLoggedIn()!.user_id;
        this.portfolioService
          .deleteSkill(userId, this.portfolioData.skills[index].id)
  ...
  openAddSkillDialog() {
    ...
    dialogRef.afterClosed().subscribe((skill) => {
      ...
      if (skill) {
        const id = this.loginService.getUserLoggedIn()!.user_id;
        console.log('id', id);
        this.portfolioService.addSkill(id, skill).subscribe((postResult) => {
